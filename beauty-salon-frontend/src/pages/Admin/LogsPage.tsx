...
function groupFromAction(action: string): ActionGroup {
  ...
  // Logowanie
  if (a.includes("AUTH") || a.includes("LOGIN") || a.includes("LOGOUT") || a.includes("PASSWORD") || a.includes("HASŁ")) {
    return "AUTH";
  }
...
export default function LogsPage(): JSX.Element {
  ...
  const hasUnappliedChanges = useMemo(() => (
    draftActionFilter !== actionFilter || draftPerformedBy !== performedBy ||
    draftTargetUser !== targetUser || draftOrdering !== ordering ||
    draftGroup !== group || draftSearch !== search
  ), [draftActionFilter, actionFilter, draftPerformedBy, performedBy, draftTargetUser, targetUser, draftOrdering, ordering, draftGroup, group, draftSearch, search]);
  ...
  const resetFilters = () => {
    ...
    setDraftActionFilter(""); setDraftPerformedBy(""); setDraftTargetUser("");
    setDraftOrdering(defaultOrdering); setDraftGroup("ALL"); setDraftSearch("");
    setPage(1); setActionFilter(""); setPerformedBy(""); setTargetUser("");
    setOrdering(defaultOrdering); setGroup("ALL"); setSearch("");
  };
  ...
  const rows = useMemo(() => {
    ...
    return base.filter((l) => {
      ...
      if (!s) return true;
      const hay = [l.action_display, l.action, l.performed_by_username, l.target_user_username].filter(Boolean).join(" ").toLowerCase();
  ...
  return (
    <Stack spacing={2} sx={{ width: "100%", maxWidth: 1200, mx: "auto", px: 2, py: 3 }}>
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 2 }}>
        <Box>
          <Typography variant="h5" fontWeight={900}>Logi systemowe</Typography>
          <Typography variant="body2" color="text.secondary">Pełna spójność z bazą danych klientów i pracowników.</Typography>
        </Box>
        <Typography variant="body2" color="text.secondary">Łącznie: {data?.count ?? "—"}</Typography>
      </Box>
      ...
      <Paper variant="outlined" sx={{ p: 2, position: "relative" }}>
        ...
        <Stack spacing={2} sx={{ pt: 1 }}>
          <Grid container spacing={2}>
            <Grid item xs={12} sm={6} md={3}>
              <FormControl size="small" fullWidth>
                ...
                <Select label="Kategoria" value={draftGroup} onChange={(e) => setDraftGroup(e.target.value as ActionGroup)}>
                  {Object.entries(GROUP_LABEL).map(([val, lab]) => <MenuItem key={val} value={val}>{lab}</MenuItem>)}
                </Select>
              </FormControl>
            </Grid>
            <Grid item xs={12} sm={6} md={6}>
              <TextField size="small" fullWidth label="Szukaj w treści..." value={draftSearch} onChange={(e) => setDraftSearch(e.target.value)} />
            </Grid>
            <Grid item xs={12} md={3}>
              <Stack direction="row" spacing={1}>
                <Button variant="outlined" fullWidth onClick={resetFilters}>Reset</Button>
                <Button variant="contained" fullWidth onClick={applyFilters} disabled={!hasUnappliedChanges}>Filtruj</Button>
              </Stack>
            </Grid>
          </Grid>
            ...
            <Table size="small">
              ...
              <TableBody>
                {rows.length === 0 && !loading ? (
                  <TableRow><TableCell colSpan={5} align="center" sx={{ py: 3 }}>Brak wyników</TableCell></TableRow>
                ) : (
                  rows.map((l) => {
                    const badge = chipPropsForGroup(groupFromAction(l.action_display || l.action));
                    return (
                      <TableRow key={l.id} hover>
                        <TableCell sx={{ whiteSpace: "nowrap" }}>{new Date(l.timestamp).toLocaleString("pl-PL")}</TableCell>
                        <TableCell>
                          <Chip size="small" color={badge.color} label={badge.label} variant={badge.variant || "filled"} sx={{ fontWeight: 600 }} />
                        </TableCell>
                        <TableCell>
                          <Typography variant="body2" fontWeight={700}>{l.action_display || l.action}</Typography>
                          <Typography variant="caption" color="text.disabled">Kod: {l.action}</Typography>
                        </TableCell>
                        <TableCell>{l.performed_by_username ? niceActor(l.performed_by_username) : "System"}</TableCell>
                        <TableCell>
                          {l.target_user_username ? (
                            <Typography variant="body2" color="primary.main" fontWeight={600}>{niceActor(l.target_user_username)}</Typography>
                          ) : "—"}
                        </TableCell>
                      </TableRow>
                    );
          ...
          <Box sx={{ display: "flex", justifyContent: "center" }}>
            <Pagination count={totalPages} page={page} onChange={(_, p) => setPage(p)} color="primary" disabled={busy} />
          </Box>
        </Stack>
      </Paper>
    </Stack>
