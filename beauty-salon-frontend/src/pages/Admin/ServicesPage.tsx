...
export default function ServicesPage() {
    ...
    function validateLocal(): boolean {
        ...
        if (form.duration_minutes.trim() === '')
            next.duration_minutes = 'Czas trwania jest wymagany.';
        else if (Number.isNaN(dur) || dur < 5)
            next.duration_minutes = 'Czas trwania musi być ≥ 5 minut.';
    ...
    async function toggleActive(s: Service) {
        ...
        try {
            ...
            const parsed = parseDrfError(e);
            setPageError(
                parsed.message || 'Nie udało się zmienić statusu usługi. Spróbuj ponownie.',
            );
        } finally {
            setTogglingId(null);
        }
    ...
    return (
        <Stack spacing={2}>
            ...
            <Paper sx={{ p: 2, position: 'relative' }}>
                {loading && (
                    <LinearProgress sx={{ position: 'absolute', left: 0, top: 0, right: 0 }} />
                )}
                    ...
                    >
                        <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap">
                            {hasActiveFiltersDraft && (
                                <Chip size="small" label="Ustawione filtry" />
                            )}
                            {hasUnappliedChanges && (
                                <Chip
                                    size="small"
                        ...
                        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
                            <Button
                                ...
                                disabled={
                                    busy || (!hasActiveFiltersDraft && !hasActiveFiltersApplied)
                                }
                            >
                                Wyczyść filtry
                            </Button>
                            ...
                            {items.map((s) => {
                                ...
                                return (
                                    <Paper
                                        key={s.id}
                                        variant="outlined"
                                        sx={{ p: 2, opacity: busy && !isToggling ? 0.75 : 1 }}
                                    >
                                        <Stack
                                            direction={{ xs: 'column', sm: 'row' }}
                                            justifyContent="space-between"
                                            ...
                                            <Box sx={{ minWidth: 0 }}>
                                                ...
                                                >
                                                    ...
                                                    <Chip
                                                        size="small"
                                                        label={
                                                            s.is_active ? 'Aktywna' : 'Wyłączona'
                                                        }
                                                        color={s.is_active ? 'success' : 'default'}
                                                        variant={
                                                            s.is_active ? 'filled' : 'outlined'
                                                        }
                                                    />
                    ...
                    <Stack spacing={2} sx={{ mt: 1 }}>
                        ...
                        <TextField
                            ...
                            helperText={
                                ...
                                'Opcjonalnie (ułatwia filtrowanie i wyszukiwanie).'
                            }
                        />
