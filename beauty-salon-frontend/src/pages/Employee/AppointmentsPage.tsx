...
import type { Appointment, AppointmentStatus, DRFPaginated, Service, Client, Employee } from "@/types";
  ...
  const handleSave = async () => {
    ...
    if (openingHours && Object.keys(openingHours).length > 0) {
      ...
      if (!isStartValid) {
        ...
        setFormError(`Wizyta rozpoczyna się poza godzinami otwarcia. Godziny otwarcia: ${periods}`);
        return;
      }

      if (!isEndValid) {
        const periods = endDayHours.map((p: any) => `${p.start}-${p.end}`).join(", ");
        setFormError(`Wizyta kończy się poza godzinami otwarcia. Godziny otwarcia: ${periods}`);
        return;
      }
    }
          ...
          <Stack spacing={2}>
            ...
            >
              ...
              <Stack direction={{ xs: "column", sm: "row" }} spacing={1} alignItems={{ sm: "center" }}>
                ...
                <Button variant="contained" onClick={applyFilters} disabled={busy || !hasUnappliedFilters}>
                  Zastosuj
                </Button>
              </Stack>
            </Stack>
                    ...
                    {(canConfirm || canCancel || canComplete || canNoShow) && (
                      <>
                        ...
                        <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                          {/* 🔥 PRZYCISK "EDYTUJ" */}
                          <Button size="small" variant="outlined" disabled={busy || isBusy} onClick={() => openEditDialog(a)}>
                            Edytuj
                          </Button>
                          ...
                          {canConfirm && (
                            <Button
                              ...
                              onClick={() => void runAction(appointmentsApi.confirm, a.id, "Wizyta została potwierdzona.")}
                          ...
                          {canCancel && (
                            <Button
                              ...
                              variant="outlined"
                              disabled={busy || isBusy}
                              onClick={() => void runAction(appointmentsApi.cancel, a.id, "Wizyta została anulowana.")}
                              startIcon={isBusy ? <CircularProgress size={18} /> : undefined}
                            >
                              Anuluj
                            </Button>
                          ...
                          {canComplete && (
                            <Button
                              ...
                              variant="contained"
                              disabled={busy || isBusy}
                              onClick={() => void runAction(appointmentsApi.complete, a.id, "Wizyta została zakończona.")}
                              startIcon={isBusy ? <CircularProgress size={18} /> : undefined}
                            >
                              Zakończ
                            </Button>
              ...
              <Stack spacing={2.5} sx={{ mt: 1 }}>
                ...
                <FormControl fullWidth required>
                  ...
                  <Select
                    ...
                    onChange={(e) => setFormData((prev) => ({ ...prev, client: Number(e.target.value) }))}
                    disabled={submitting}
                  >
                    {clients.map((c) => (
                ...
                <DateTimePicker
                  ...
                  slotProps={{
                    textField: {
                      fullWidth: true,
                      required: true,
                      helperText: formData.service && formData.start
