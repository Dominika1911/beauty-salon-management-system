...
const parseYyyyMmDdLocal = (s: string): Date => {
  // input type="date" daje YYYY-MM-DD; tworzymy lokalną północ bez ryzyk timezone z Date.parse
...
export function BookAppointmentPage(): ReactElement {
  ...
  const refreshSlots = async (): Promise<void> => {
    ...
    try {
      ...
      // Jeśli aktualny dzień-tab nie mieści się w nowym zakresie -> ustaw na pierwszy dzień zakresu
      const inRange = dateRange.days.some((d) => d.key === activeDayStr);
      if (!inRange) {
        setActiveDayStr(dateRange.fromStr);
      } else {
        // jeśli aktywny dzień ma 0 slotów, a inne mają – UX: przeskocz na pierwszy dzień z slotami
        const anyForActive = newSlots.some((s) => s.start.startsWith(activeDayStr));
        if (!anyForActive) {
          const firstDayWithSlots = dateRange.days.find((d) => newSlots.some((s) => s.start.startsWith(d.key)));
  ...
  // zmiana dateFrom -> przestaw aktywny dzień na pierwszy w zakresie (spójny UX)
  useEffect(() => {
  ...
  const slotsByDay = useMemo(() => {
    ...
    for (const s of slots) {
      // start to ISO string; YYYY-MM-DD na początku (w formacie isoformat z backendu)
      const key = s.start.slice(0, 10);
      const list = map.get(key);
      if (list) list.push(s);
    }
    // sortowanie w obrębie dnia
              ...
              <label style={{ display: 'block', marginBottom: 8 }}>
                ...
                {serviceId !== '' && filteredEmployees.length === 0 ? (
                  <div style={{ ...beautyMutedTextStyle, marginTop: 6 }}>
                    Brak aktywnych pracowników wykonujących wybraną usługę.
                  </div>
                ) : null}
              </label>
