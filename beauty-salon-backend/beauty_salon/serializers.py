...
class UserListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['id', 'username', 'first_name', 'last_name', 'email', 'role', 'role_display', 'is_active', 'created_at']
...
class EmployeeSerializer(serializers.ModelSerializer):
    ...
    #  NOWE: statystyki z annotate w EmployeeViewSet.get_queryset()
    ...
    def update(self, instance, validated_data):
        ...
        # opcjonalnie: synchronizacja danych usera
        user = instance.user
...
class BookingCreateSerializer(serializers.Serializer):
    ...
    - CLIENT rezerwuje dla siebie (z request.user.client_profile)
    ...
    def validate(self, data):
        ...
        if role == "CLIENT":
            if not hasattr(user, 'client_profile'):
                raise serializers.ValidationError('Użytkownik nie ma profilu klienta.')
        ...
        if Appointment.objects.filter(employee=employee, start__lt=end, end__gt=start).exclude(
            status=Appointment.Status.CANCELLED
        ).exists():
            raise serializers.ValidationError('Ten pracownik ma już zajęty termin.')

        if Appointment.objects.filter(client=client, start__lt=end, end__gt=start).exclude(
            status=Appointment.Status.CANCELLED
        ).exists():
