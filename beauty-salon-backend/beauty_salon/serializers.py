...
def _generate_internal_username(prefix: str) -> str:
    """
    Generuje username zgodny z CustomUser.USERNAME_PATTERN (models.py),
    np. pracownik-1234 albo admin-1234.
    """
    while True:
    ...
    def create(self, validated_data):
        ...
        with transaction.atomic():
            if "user" not in validated_data:
                # ✅ zgodne z USERNAME_PATTERN: pracownik-1234
                username = _generate_internal_username("pracownik")
...
class ClientSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        with transaction.atomic():
            if "user" not in validated_data:
                # ✅ Klient może mieć pusty username (model na to pozwala)
                # Jeśli bardzo chcesz login dla klienta, musisz zmienić USERNAME_PATTERN w modelu.
                user = User.objects.create(
                    username="",
                    email=email,
                    first_name=validated_data.get("first_name", ""),
    ...
    def update(self, instance, validated_data):
        ...
        if (email or password) and instance.user:
            user = instance.user
            if email:
                user.email = email
            if password:
    ...
    def validate(self, data):
        ...
        if employee and service:
            if not employee.skills.filter(id=service.id).exists():
                raise serializers.ValidationError({"service": "Ten pracownik nie wykonuje wybranej usługi."})

        # ✅ spójność end z czasem trwania usługi (bez bufora)
        if start and end and service:
            expected_end = start + timedelta(minutes=int(service.duration_minutes))
            if end != expected_end:
                raise serializers.ValidationError({"end": "Czas zakończenia musi wynikać z czasu trwania usługi."})

        # konflikty pracownika (bez bufora — bufor i tak blokuje booking/sloty)
...
class BookingCreateSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        date = start.date()
        if TimeOff.objects.filter(employee=employee, date_from__lte=date, date_to__gte=date).exists():
